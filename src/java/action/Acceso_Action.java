...
public class Acceso_Action extends ActionSupport implements SessionAware {
    ...
    public List<ciclosBean> cicloActivo = new ArrayList<ciclosBean>();
    //SESSIN USUARIO	
    ...
    public String MuestraMenuPrincipalUsuario() {
        ...
        try {
            ...
            if (usuariocons != null) {
                ...
                cicloActivo = acceso.cicloAct();

                Iterator cA = cicloActivo.iterator();

                ciclosBean obj;

                while (cA.hasNext()) {
                    obj = (ciclosBean) cA.next();

                    usuariocons.setID_CICLO(obj.getID_CICLO());
                    usuariocons.setFECHA_INICIO(obj.getFECHA_INICIO());
                    usuariocons.setFECHA_TERMINO(obj.getFECHA_TERMINO());
                    usuariocons.setCICLO(obj.getCICLO());

                }

                session.put("usuario", usuariocons);

                Constantes.enviaMensajeConsola("EL CICLO ES " + usuariocons.getCICLO());
                ...
                if (usuariocons.getESTATUS().equals("0")) {
                    addActionError("Usuario restringido, contacta al Administrador");
                    return "ERROR";
                }
    ...
    public List<ciclosBean> getCicloActivo() {
        return cicloActivo;
    }

    public void setCicloActivo(List<ciclosBean> cicloActivo) {
